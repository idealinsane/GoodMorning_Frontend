...
class LogInPage extends StatelessWidget {
  ...
  Future<Map<String, dynamic>> getUserProfile() async {
    try {
      ...
      // iOS 에뮬레이터용
      final url = 'http://localhost:9090/api/users/me';
          ...
          InkWell(
            onTap: () async {
              try {
                ...
                try {
                  final verificationResult = await verifyTokenWithBackend(
                    userCredential,
                  );
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('로그인 성공: ${verificationResult["email"]}'),
                    ),
                  );
                } catch (e) {
                  ...
                  ScaffoldMessenger.of(
                    context,
                  ).showSnackBar(SnackBar(content: Text('토큰 검증 실패: $e')));
                }
              } catch (e) {
                print('Login error: $e');
                ScaffoldMessenger.of(
            ...
            onPressed: () async {
              try {
                ...
                if (FirebaseAuth.instance.currentUser == null) {
                  ScaffoldMessenger.of(
                    context,
                  ).showSnackBar(SnackBar(content: Text('먼저 로그인해주세요')));
                  return;
                }
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
                      '프로필 정보: ${profileData.toString().substring(0, 100)}...',
                    ),
                  ),
                );
              } catch (e) {
                ...
                ScaffoldMessenger.of(
                  context,
                ).showSnackBar(SnackBar(content: Text('프로필 정보 가져오기 실패: $e')));
              }
            },
